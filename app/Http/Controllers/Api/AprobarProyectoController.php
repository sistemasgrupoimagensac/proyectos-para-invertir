...
class AprobarProyectoController extends Controller
{
    public function aceptar_proyecto(Request $request)
    {
        ...
        try {
            ...
            $max_prioridad_proyecto = InversionistaProyecto::where('prestamo_id', $prestamo->co_prestamo)->max('prioridad');
            $prioridad = $max_prioridad_proyecto ? $max_prioridad_proyecto + 1 : 1;

            $exist_inversionista = InversionistaProyecto::where([
                    'prestamo_id' => $prestamo->co_prestamo,
            ...
            if ( $proyectoAsignado ) {
                ...
                $persona = RPrestamoInversionista::join('p_inversionista AS pi', 'pi.co_inversionista', 'r_prestamo_inversionista.co_inversionista')
                    ...
                    ->where('r_prestamo_inversionista.co_prestamo', $prestamo->co_prestamo)
            ...
            $cant_aprobados_plataforma = InversionistaProyecto::where('prestamo_id', $prestamo->co_prestamo)
            ...
            if ( isset($max_prioridad_proyecto) ) {
                ...
                $existe_cola = InversionistaProyecto::where([
                    ...
                    ->where('persona_id', '<>', $request->user()->inversionista_id)
                ->first();
                
                if ( $existe_cola ) {
                    Mail::to($p_inversionista->no_correo_electronico)->cc($analistas_emails)->send(new NotificacionProyectoAprobadoCola($prestamo->co_unico_solicitud, $p_inversionista->no_completo_persona, $analista->name, $request->proyecto_id, $prioridad));
                    ...
                    return response()->json([
                        ...
                        'message' => "Proyecto aprobado correctamente, Estas en cola para el proyecto, eres el número: " . $prioridad,
                        'success' => true,
                    ]);
                    ...
                    ->first();
                    if ( $proyectoAsignado ) {
                        Mail::to($p_inversionista->no_correo_electronico)->cc($analistas_emails)->send(new NotificacionProyectoAprobadoCola($prestamo->co_unico_solicitud, $p_inversionista->no_completo_persona, $analista->name, $request->proyecto_id, $prioridad+1));
                        ...
                        return response()->json([
                            ...
                            'message' => "Proyecto aprobado correctamente, Estas en cola para el proyecto, eres el número: 2",
                            'success' => true,
                        ]);
                    } else {
                ...
                ->first();
                if ( $proyectoAsignado ) {
                    Mail::to($p_inversionista->no_correo_electronico)->cc($analistas_emails)->send(new NotificacionProyectoAprobadoCola($prestamo->co_unico_solicitud, $p_inversionista->no_completo_persona, $analista->name, $request->proyecto_id, 2));
